        ...
        async with aiohttp.ClientSession(headers=await twitch_auth.get_headers(self.db)) as session:
            async with session.get(
                ...
                if resp.status != 200:
                    ...
                    log.error('Failed to list subscriptions for %s: %d', twitch_user_id, resp.status)
                    return
            ...
            for sub in data.get('data', []):
                ...
                async with session.delete(
                    ...
                    if resp.status == 204:
                        ...
                        log.info('Cancelled subscription %s for %s', sub["id"], twitch_user_id)
                    else:
                        print(f'[EVENTSUB] Failed to cancel subscription {sub["id"]}: {resp.status}')
                        log.error('Failed to cancel subscription %s: %d', sub["id"], resp.status)
